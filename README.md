...
上传校对与两个管理员工作台共享科学数据来源核对。管理员编辑页顶部提供独立「进行 AI 审核」按钮，先保存当前编辑再核对论文及附件；提交审核只检查结果，不自动调用模型。问题字段保留红框，点击框左上角现有中英文说明文字打开右侧抽屉；同框问题聚合，不再显示红点，依次查看当前内容、问题与方向、可编辑的 AI 建议。管理员直接改值后，点击「完成」先保存主表单，再保存绑定当前版本的人工确认草稿；AI 候选仍在提交时应用，完成不自动批准。未核对项目与没有出处分开展示；管理员可对推理或专业判断逐条填写理由完成确认，来源标注为人工判断，不冒充论文或 AI 结论。未保存新值的理由先留在本地，失败和迟到响应不覆盖输入；保存成功的核对结果与建议草稿持久保存在 MySQL，刷新可恢复。正式批准才写入永久来源及审核历史，并清理临时判断。
