...
需要恢复数据时，把 `make frozen` 生成的一个 `.tar.gz` 迁移包放到源码根目录 `dist/`，
直接执行上述命令即可自动检查、解压并进入恢复流程，无需手填文件名。

- `dist/` 只有一个压缩包：自动选择；配套 `.sha256` 不计数。
- 多于一个：报“必须只能保留一个数据库压缩包”，先移走多余包再重试；不选择最新或自动删除。
- 没有压缩包：复用已解压的 `.deployment`；两者都没有才初始化空库。
- 坏包、不支持的 ZIP/裸 SQL 压缩文件、源码/服务版本不一致或已有其他实例：在安装前拒绝，
  不改 `.env`、源码、数据库或既有部署报告，不悄悄改成空库部署。
