...
# 2026-09-10 收敛：元素种类数临时输入

在 #101 全量回归中发现并通过独立 HEAD 副本复现 [Issue #102](https://github.com/JLU-ICCMS-MaYuan/SC-Wiki/issues/102)：非法元素种类数只修改 `elementCountEdits`，旧缓存未比较它，导致提示被旧卡片遮蔽。

修正方式：将当前下标的临时输入对象加入该卡片缓存比较与记录。只失效本张卡片，保留其他卡片和父级草稿引用；不扩大缓存清空范围、不改变校验和保存契约。既有完整上传编辑器分类用例验证保存载荷，新增五张卡片用例验证局部渲染。
