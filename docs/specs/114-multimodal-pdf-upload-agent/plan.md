...
在现有 RQ 上传 Worker 内增加解析适配器层和统一 Document IR。Docling 负责默认结构化解析，MinerU 和 PDF 原生多模态 LLM 处理复杂页面或跨页理解，PyMuPDF 作为无视觉能力时的 fallback。Claim、Evidence 和 CoverageReport 在进入现有草稿/科学数据契约前由程序校验。正式提交时保存文档定位数据，前端按需从原 PDF 渲染区域。
...
- **数据存储**：Redis 保存任务运行态；任务 artifact 保存临时 IR/Claim；MySQL 保存正式文档块、Evidence 和论文 revision 关联；原 PDF 保持现有文件存储。
- **测试体系**：pytest、真实隔离 Redis/RQ、SQLite/MySQL 回归、Vitest、浏览器夹具、Go 测试和 50 篇论文基准。
- **目标平台**：本地 CPU、可选 GPU、Docker Worker；无 GPU 时必须可启动并明确降级。
...
| FR-010 / SC-004 | 失败可见、可重试或人工处理 | 统一 ParserRun 状态、fallback 原因和 RQ 失败收敛 | 通过 |
| FR-011 / SC-008 | Shadow、灰度、默认可配置 | rollout 配置和任务级 fallback，不让用户选择解析器 | 通过 |
| FR-012 | 正式定位数据支持 revision | Alembic 迁移、复制服务和删除回归 | 通过 |
| AGENTS.md Git | 修改后合理验证并自动提交，不能混入其他改动 | 仅暂存 #114 文档/代码/测试，验证后使用规定多行提交信息 | 待提交 |
...
**结构选择**：解析器、IR、Claim 校验和覆盖审计保持单一职责；`upload_jobs.py` 只负责编排和现有草稿转换。正式定位数据归论文 revision 所有，不在前端复制科学事实。区域图片由原 PDF 和定位数据按需生成，不增加截图存储表。

## 需求到设计的映射

| 来源 | 设计组件/接口 | 验证方式 |
| --- | --- | --- |
| FR-001～FR-004 / US1 | `DocumentParser`、`DocumentIR`、`DocumentBlock` | parser 契约、数字/扫描/表格样例 |
...
| FR-010 / US3 | `ParserRun`、RQ 失败收敛和 fallback | 隔离 Redis/RQ 任务测试 |
| FR-011 / US5 | `RolloutConfig`、shadow/gray/default 路由 | 配置、回退和质量门测试 |
| FR-012 / US4 | 文档定位 ORM/迁移、revision 复制 | MySQL/SQLite 迁移和删除回归 |
| FR-014 / US2/US4 | 现有 `upload_jobs.py`、`scientific_evidence.py`、前端 Evidence 工作流 | 真实提交/审核回归 |

## 实施阶段

1. **Setup**：锁定依赖版本、许可证、配置和 50 篇评测集清单。
2. **Foundational**：建立 IR、解析器协议、fallback、Claim、Evidence、Coverage 和 rollout 配置。
3. **US1**：接入 Docling、MinerU、PyMuPDF 和 PDF LLM 适配器，保持旧链路可用。
4. **US2**：接入 Claim 提取、证据校验、现有草稿转换和正式文档定位迁移。
5. **US3**：接入覆盖审计、有限 Agent 工具、失败收敛和可重试/人工状态。
6. **US4**：正式保存定位数据，增加区域证据 API 和校对/审核 UI。
