...
返修或管理员编辑采用新引句时，共享保存服务在当前 IR 中重新建立定位关联；只给
同论文、同当前 revision、同文件且可定位的引句关联区域。没有 IR 的旧论文保留原
文本证据。科学记录的计算/实验条件写入各自 payload，不通过全文汇总重新生成。

`20260924_0114` 是后续目录数据迁移，补齐已支持 Tc 方法的版本 1 表单定义，包括
measured_tc.unknown；不修改已发布定义或已有记录。回退应用时保留目录定义，以免
删除已经被科学记录引用的契约。该迁移不改变 #114 已部署定位表的结构。
