...
- [x] T015 [US2] 将 Document IR 块和表格单元接入现有分段/汇总提示输入，输出版本化 Claim。
- [x] T016 [US2] 在 upload_jobs.py 中调用 Claim 定位校验并映射现有 field_path、basis_kind、source_kind 和 Evidence。
...
- [x] T019 [US2] 在提交事务和 revision 服务中把文档定位复制到新的当前 revision，保持旧 revision 不作为完整文档打开，并补齐真实数据库回归。
...
- [x] T028 [US5] 实现 Shadow 结果存储、旧/新结果对照摘要和不影响用户草稿的任务分支。
...
## 2026-09-24：领域提取、真实事务与 Shadow

- T015/T016：新 PDF 输入完整 IR 页/块/表格，版本 1 领域输出直接采用模块化记录；
  未知方法不猜测。科学值由后端组装，全文 LLM 不重新生成记录，多材料/压力/方法/
  条件、跨页证据和 TXT 附件来源均有反例验证。
- T019：真实 HTTP/JWT、RQ、MySQL 迁移与事务、Go 拒绝/批准、上传者返修升版通过。
  采用新引句时重建定位；提交时 IR 哈希错误整体回滚并恢复 ready。模型响应固定注入，
  向量发布使用替身，这不是论文提取质量或向量索引验收。
- T028：独立 Shadow 作业、文件/凭据/产物隔离、只保存对照、不进入用户列表与提交，
  以及对照失败/父任务取消已通过真实 RQ 验证。并行执行需要多个 Worker。
- T030：批量 API 上传、轮询、实际 AI 产物与区域导出、逐篇落盘已接通；真实 50 篇
  人工标注与供应商质量报告仍缺，保持未完成，默认 legacy 不变。
- T001/T012/T014/T021～T024/T027/T029/T031 及最终跨模块验收仍按各自范围保留，
  不因这轮端到端用例通过就宣布整个 Feature 完成。

## 2026-09-24：H3S 单篇实测收敛

依据 [正文与补充材料实测](case-studies/2014-h3s.md)，先用本例验证字段证据，不要求本轮
完成 50 篇基准，也不据单篇结果开启默认新链路。以下为实测后新增的技术拆解，本次
仅记录问题，没有修改生产解析器或放宽校验。

- [ ] T037 [US1] 压缩重复 IR 元数据并按表格行/块分段，保留列头、相、压力和跨页上下文；
  使本例补充第 4 页的 Docling/MinerU 输出进入领域读取，而不是超过 60,000 字符后停止。
- [ ] T038 [US2] 对真实自动填表结果逐字段核对数值、单位、符号、材料/相、压力与方法；
  以 Tc=999 K、错挂 λ 和斜率变号反例区分定位通过与科学语义支持，保留最终人工审核。
- [ ] T039 [US3] 覆盖审计记录引用/背景、非 Tc 结构表、装饰图和需视觉复核等不同处置原因；
  不把每个结构表行都要求为 Tc，也不通过忽略图像伪造 complete。
- [ ] T040 [US1/US4] 针对本例文本层等号/负号/乘号失真、OCR 化学式错误、S1 跨页归属与
  单元坐标缺失建立真实样例回归；源图复核后的修正必须保留原解析文本与修正依据。
