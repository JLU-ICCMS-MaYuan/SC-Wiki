...
## 2026-09-24：领域提取、端到端、Shadow 与批量驱动

版本 1 领域候选直接使用模块化记录，完整 IR 页/块/表格进入分段输入；科学候选由
后端组装，汇总模型只生成书目与叙述。测试覆盖不同材料、压力、方法、判据、条件、
多页多引句，以及任一无效引句不能被其他有效引句掩盖、TXT 附件来源不能丢失。

### 定向回归

完整 #114 单元/适配契约与上传作业、草稿、Redis 仓库、凭据和迁移头检查：
**193 项通过、5 项跳过**。跳过包含显式启用的实际 SDK、专用 MySQL 增量和独立 Redis
入口测试；既往实际 SDK 验证仍见上文，本轮 MySQL/RQ 则由下述独立流程执行。
扩大测试最初发现 6 个旧测试失败，在原提交 `1d9d808` 隔离副本复现相同失败后，修正
纯单元测试的持久层隔离夹具、非必需书目项与材料必填错误码断言，不降低业务门禁。

### 实际隔离流程

`python scripts/run_issue114_integration.py`：**8 项通过**。新建 MySQL 8.4 临时实例，
只监听 Unix socket；完整运行 Alembic 与 #90 空库转换阶段，最终执行两条迁移分支。
Redis 同样使用独立临时实例；没有使用业务库作为测试库。

验证链：真实 HTTP/JWT 上传 → RQ → PDF/IR/领域契约/证据门禁 → 保存校对草稿 →
未核对提交被拒绝 → 正式事务提交 → Go 管理员拒绝 → 上传者返修并换用原文引句 →
升版为当前 revision 2 → 管理员保存分类、核对 → Go 批准。核实物性与区域定位仍归当前
revision，上传者不能自行审核。新增引句关联、重复提交、区域显示、IR 哈希错误整体
回滚和任务恢复 ready 均通过。

另外验证：领域 JSON 非法后的真实排队重试、Shadow 成功与解析器不可用、内部任务
不能访问/补证/提交且无科学数据库写入、文件快照不影响原件、父任务取消、Shadow
入口导入失败回调清理凭据并收敛状态，以及批量驱动真实上传后导出两个压力记录和
正确 PDF 区域。Go 定向测试编译生产源码与本轮验收文件，未混入同时开发的 #115 测试。

**边界**：模型响应固定注入，GROBID 不可用响应和向量发布使用替身；不代表真实供应商
科学准确率、向量索引或浏览器验收。批量驱动测试使用合成 PDF，不能计为人工论文。

### 本机增量

已应用 `20260924_0114`，当前版本为 `20260914_0052`、`20260924_0114`。只新增 9 个
缺失 Tc 方法定义，核对已有定义 ID/校验和保持不变；未改写科学记录或启用默认新链路。
已发布定义不会随应用回退删除。其他部署实例仍需明确执行该增量。

### 尚待真实验收

真实 50 篇人工标注集、供应商/模型配置与质量报告尚未提供；因此未运行真实论文基准，
未切换默认，也不关闭 #114。原生 PDF、视觉、完整灰度回退、浏览器及模型许可等各自
剩余任务仍按 Tasks 保留。

## 2026-09-24：用户提供的 H3S 单篇试验

按用户要求，暂以正文 6 页和补充材料 10 页进行单篇对比，详见
[完整报告及逐字段清单](case-studies/2014-h3s.md)。三种本地解析器共 6 次实际解析均成功，
原 PDF 摘要未变，未提交或发布科学数据。

复现的重要边界：Docling/MinerU 的补充第 4 页结构化输入分别为 168,177/208,526 字符，
超过当前 60,000 字符单页预算；文本层把负号/等号/乘号读坏，OCR 也出现化学式和列
合并错误。9 个错误值或条件候选仍通过仅定位校验，不能据此证明字段语义受原文支持。

助手依据原图整理了四组 Tc 与 77 项核对点，并保存每个解析器的原文定位和参考草稿。
这不是后台 LLM 自动填表：本轮服务端默认模型没有可用 API Key，没有执行真实模型
语义核对，不把参考草稿计为模型正确率或人工标注质量门。实测待办追加为 T037–T040。
