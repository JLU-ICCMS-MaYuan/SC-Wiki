...
- [ ] T034 [US2] 修复 MySQL 调度 ON 但无事件时误拒绝；停服务前只读核验全实例事件与线程、管理员可见性和实例 UUID，导出前后与发布前复核。补齐无事件/禁用/启用/运行中/权限不足/错误实例回归，并实际执行用户当前实例 `make frozen`、检查包与源服务恢复。来源 FR-009/014/017。
