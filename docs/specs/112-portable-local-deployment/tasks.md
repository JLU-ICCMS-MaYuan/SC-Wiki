...
用户已把“最新包”改为“只允许一个包”。以下任务补齐 T020 自动发现的缺口，不据此
完成尚待确认的跨源码版本恢复或授权覆盖当前实例。CHK019 仅影响 Docker 目标。

- [x] T041 [US3] 在 `tests/02_maintenance_and_verification/test_local_bundle_discovery.py` 增加真实 Make 入口的零包、唯一包/校验文件、多包、坏包、来源冲突、源码不兼容、非空目标及重复选择保护；实际解包与校验，禁止测试访问当前业务库。来源：FR-020、SC-008，T020 缺口；新增 31 项通过，见 validation.md。
- [x] T042 [US3] 在 `scripts/local_deploy/bundle.py`、`cli.py`、`state.py` 实现唯一包自动发现、统一准备/恢复路径和写入前的状态归属检查；多包与坏包报告只读阻断，不自动删除、降级为空库或覆盖既有数据。来源：FR-011/014/020、SC-004/008；91 项回归通过、11 项跳过，真实 Make 已选中当前包并准确阻断源码冲突，不代表当前数据已恢复。
