...
- 用户准备 Conda、Bash、Make、Python 3、curl 与 CA 证书、tar/gzip、ss、setsid 和基础文件/文本/进程工具；源端有 Git。本地目标不需要 Docker；Docker 目标不属于本 Feature。
...
预期：首次检查给出真实缺项而不写文件；部署使用用户已有 Conda 准备 `sc-wiki`、全部依赖、新 `.env`、空业务库及 118 个元素，不修改 base。网页入口为 `http://127.0.0.1:5173`。没有默认网站账号，部署输出指向既有 `backend.scripts.create_superadmin`，并提供在已加载本机环境中调用它的方法。

第二次部署不得改变 `.env` 摘要或重复初始化。保存三个环境样本：缺少 Conda 时只读阻断；已有 Conda 而无 `sc-wiki` 时创建；已有非默认前缀且兼容的 `sc-wiki` 时复用。另测不可执行的 `CONDA_EXE`、查询失败、base 被命名或链接为 `sc-wiki` 均拒绝，且不修改 base。
...
```bash
make frozen OUTPUT="/tmp/sc-wiki-migration.tar.gz"
sha256sum -c "/tmp/sc-wiki-migration.tar.gz.sha256"
```

预期：生成仅含数据的冻结包和外部校验文件；停写结束后源服务运行状态恢复，源库 schema 和数据未被迁移命令更改。检查全表/对象、图、向量及文件清单；包内不应出现源码、`.git`、源 `.env`、依赖安装目录或任何植入的假密钥。
...
在已 clone 且与数据包源码身份一致、尚未部署的隔离源码目录，将场景 B 的压缩包和 `.sha256` 放入根目录
...
## 场景 E：同一个包恢复到 Docker（已移出 #112）

使用场景 B 的同一份包，在另一个空隔离目录执行拟新增的 `make dockerdeploy`，
验证包摘要不变、无需 Conda、应用镜像由目标源码构建。恢复前不得启动应用或 Worker；
恢复后的正常启动及重跑只核验数据库，不执行历史迁移。

逐项复用场景 C 的表、表单定义、审核历史、原账号登录、附件、草稿、图/向量验收；
特别检查容器内 `/data` 与宿主附件挂载一致，以及同包重跑保留目标新增数据。
重复场景 D 的非空目标、损坏包、中断与源码不匹配测试。Docker 源实例打包不在本轮范围。
Docker 恢复入口不属于 #112；本场景仅保留历史需求记录，不是本 Feature 的验收项。

## 验收记录要求

记录系统版本/架构、源码提交、包标识、依赖版本、测试场景结果、耗时/空间、失败重试证据，以及未配置/未验收的外部能力。不记录凭据和真实论文内容。2026-09-23 用户已确认 WSL 到 Ubuntu 的手动验收和当前 Ubuntu 实例部署完成，详见 validation.md。
