...
- 用户准备 Conda、Bash、Make、Python 3、curl 与 CA 证书、tar/gzip、ss、setsid 和基础文件/文本/进程工具；源端有 Git。本地目标不需要 Docker；只有场景 E 的独立 Docker 目标需要它。
...
当前入口尚未实现；此处是 T033 的验收要求，不是可执行的成功记录。
