...
用户确认已完成从 WSL 到 Ubuntu 的手动跨机器验收；当前 Ubuntu 机器上运行的 SC-Wiki 即按本 Feature 流程完成部署。该现场验收覆盖：

- **T022/T023**：跨用户名、跨路径、干净 Ubuntu/WSL2 目标的完整打包、恢复、服务启动和业务访问检查。
- **T026–T028**：无 Conda/环境冲突、故障处理、全业务样本、审核历史、结构候选附件和真实迁移链路验证。
- **T031–T033**：根据用户最终确认从 #112 移出范围；本 Feature 不实现 Docker 目标恢复，后续如有需要另建 Issue。

因此本记录的最终状态为：本地部署与跨机器恢复验收已完成，Docker 目标不属于本 Feature。文档后方较早日期的“尚未完成”描述保留为历史验证记录，不再代表当前关闭判断。
...
该段记录截至 2026-09-22 的待验收状态；2026-09-23 用户已确认相关现场验收完成，当前关闭判断以文档开头的最终人工验收结论为准。

## 用户调整验收及提交要求

2026-09-21，用户明确表示不需要 Ubuntu 24.04 验收，并要求先提交 Git 记录。
该平台测试已从必需验收中移除；此前镜像拉取的代理超时不再作为阻塞。本次依据当时已通过的验证提交当前实现与文档；后续现场验收结论见文档开头的最终人工验收结论。
