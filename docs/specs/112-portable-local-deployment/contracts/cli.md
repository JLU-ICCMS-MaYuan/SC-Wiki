...
脚本返回：0 为基础流程成功，2 为参数/平台/前置条件错误，3 为包校验或版本不兼容，4 为目标冲突，5 为安装/导出/恢复/启动失败。GNU Make 本身可能把配方的非零状态统一为 2，调用方不能从 Make 的退出码区分细类；稳定细类由结构化报告的 `error_code` 表达。

报告写入本机 `.local/deployment-report.json`，控制台给出同义中文摘要。字段包括操作、结果、完成阶段、错误码、源码/包标识、数据验证计数、各服务状态、访问地址、缺少的外部配置及下一步。预检模式只向标准输出报告，不写该文件。
