...
- 打包不导出或启停 GROBID；保留数据库与应用进程归属检查。本地部署启动检查 GROBID 的本机进程组归属。
...
- 停服务前检查 MySQL 全实例事件、执行线程及文件目录；调度开启但无启用事件和执行线程时允许继续。检查连接必须有全局 EVENT 可见权限并与业务连接属于同一实例，导出前后和发布前复核；不修改调度开关。启用事件仍须先确认处理，不按预计下次时间自动放行。
...
# 可选：在已 clone 且与数据包兼容的源码目录显式选择一个包
...
- 默认扫描根目录 `dist/`：只有一个压缩包则自动选择、校验并解压恢复，多个则报“dist 中必须只能保留一个数据库压缩包”，不选择最新、不自动删除；`.sha256` 不计数。接受 `make frozen` 生成的可校验 tar 迁移包，普通 ZIP 或裸 SQL 压缩文件不受支持，不能回退空库。
- `BUNDLE` 可覆盖自动选择的来源，但不绕过 `dist` 多包限制；与既有 `.deployment` 或部署记录冲突时拒绝。没有压缩包时复用 `.deployment/manifest.json`，两者都没有才初始化空实例。
- 发现包后仍核验 `source_identity`、服务版本、清单和空目标，不自动替换源码或 `.env`。包校验、多包与目标冲突均在安装前只读阻断；已有实例即使最初为空，也不能被另一个包直接覆盖。
...
缺少可选外部凭据不阻止基础服务成功，但不能写成“全部功能已验证”。任一必要数据库或本机 GROBID 不可用、恢复校验失败或 Worker 未就绪会使基础部署失败。Docker 是否安装或运行不影响本地部署。
