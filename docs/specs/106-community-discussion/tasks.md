...
| FR-005、FR-013 | T013–T015 |
| FR-009、FR-010 | T007、T009 |
| FR-011 | T002、T003、T007 |
| SC-001–SC-005 | T009、T010；SC-002 由 T013–T015 更新 |
...
- 上述完成指代码与隔离验收；本地启用及当前账号实际发布确认见下方追加任务，Issue #106 保留协作状态。

## 本地启用收敛

- [x] T011 经用户明确确认，对正在运行的 Go 所连接的本地 scwiki 数据库执行 20260917_0107，保留并行分支；核验六张表、实际读取接口、回滚写入与浏览器页面，并回写验收和 Overview。
- [x] T012 用户确认当前账号在原页面已发布成功；回写本地使用验收及 Overview，区别用户反馈、自动化验证和其他环境部署。

## 取消弹幕收敛

用户要求取消全部弹幕：与评论重叠、无法盖楼连评、输入区占用空间；保留评论及回复，历史弹幕数据留库且不外露。原 T006 和上述弹幕验收仅为历史实施记录。

- [x] T013 移除全部前端弹幕挂载、组件、类型和中英文文案，保留体系及三个论文入口的共用评论。
- [x] T014 收紧 Go 内容类型，拒绝旧弹幕请求，过滤历史记录的按 ID 访问、操作、通知与管理举报；补充行为回归。
- [x] T015 更新浏览器与前端测试，验证现有运行页面及评论回复，同步 Issue #106、当前 Spec、Overview 与 README 并按任务范围提交。
