...
- 内容使用带类型和明确外键的统一表，避免三套作者、软删除和治理逻辑；体系空间使用 system_key 主键。通知在内容事务内创建，点赞唯一约束保证幂等。
- 前端新增 `frontend/src/components/community/` 和 `frontend/src/pages/DiscussionPage.tsx`、`SystemCommunityPage.tsx`；公共客户端只传明确目标。评论组件负责分页、回复、编辑和登录入口。
...
- 不缓存权限敏感内容和通知；只保留站内通知未读数轮询。删除 DanmakuPanel 及全部挂载、类型和专用文案，内容入口只接受问题、答案和评论；按 ID 读取/操作、通知及管理举报查询统一排除退役类型。历史数据保留，不新增迁移。
...
| FR-005、FR-013 | 移除弹幕，T013–T015 | 旧接口拒绝、历史数据不外露、页面无弹幕请求、评论回复回归 |
| FR-009、FR-010 | 通知与治理，T007 | 事务通知、已读及举报处理 |
| FR-011 | 唯一约束、限流、身份映射，T002、T003、T007 | 并发点赞、封禁、注销、超限 |
| SC-001–SC-005 | T009、T010；SC-002 由 T013–T015 更新 | 集成和浏览器记录 |

文档门：无未决高影响决策；产物齐全；需求均映射到任务和验收。实现门：真实生产路径、行为测试和隔离集成验证，不用源码文本断言代替行为。关闭门：Overview、README 与验收完成。当前移除范围按 FR-005、FR-013 → 类型约束及共用评论 → T013–T015 → Go 行为、浏览器和文档检查追踪。

## 实施顺序与边界

文档 → 数据与权限 → 问答 → 详情评论 → 通知管理 → 集成与文档。共享文件串行修改。工作区已有其他修改，记录起始快照并只暂存本次差异。不创建或切换分支，不推送。现有数据库迁移需在脚本和验证结果可审查后单独确认。
