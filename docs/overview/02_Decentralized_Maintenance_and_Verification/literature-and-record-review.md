...
- 已落库结构的晶胞表示（惯用胞/原胞）与格式（CIF/POSCAR）按需从落库结构的实际格式生成（`GET /api/rag/papers/{id}/structures/{sid}/representations`），管理端编辑页可自由切换预览并下载对应文件。既有 POSCAR 按真实格式回传，服务端转换后校验，不将 POSCAR 原文当作 CIF。（[Issue #78](https://github.com/JLU-ICCMS-MaYuan/SC-Wiki/issues/78)、[Issue #110](https://github.com/JLU-ICCMS-MaYuan/SC-Wiki/issues/110)）
...
- 保存前按语义比较当前数据，完全无变化时不重建、不升版、不新增修改历史；仅删除结构也算有效修改。重建保留未修改结构的原格式、原文、计算方法、参数和来源，重新绑定父子关系与物性结构引用；删除的结构不会恢复，指向已删除结构的引用清空。编辑结构文本后不继承旧结构的物理元数据或来源。既有结构必须属于当前论文且未被重复引用，旧身份或外部论文身份返回 409。
- 重建前的物性定义事件归档到论文修改历史；同版本且物性记录本身未改变时，活动定义事件保留原事件身份并映射到新记录，可沿合法事件链回滚。已升版、已修改或已删除记录的旧事件只供追溯；回滚同时核对最新事件、记录摘要和完整快照，过期请求返回 409。同次 Go/Python 保存共用历史操作 ID，科学保存补齐审计快照及版本，不重复新增事件。
- 管理员编辑页与快速审核建议保存共用结构转换；保存响应提供新旧结构身份映射，独立编辑页据此支持同页连续保存，并保留等待响应期间新增的输入、候选与删除选择。上述保留、映射和历史写入与科学重建处于同一事务，失败全部回滚。详见 [#110 保存契约与验收记录](../../specs/76-review-scientific-data-editing/contracts/scientific-draft-api.md)。
