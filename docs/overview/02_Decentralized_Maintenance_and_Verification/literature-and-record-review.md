...
| 进行 AI 审核 | 先保存编辑，再核对当前论文及附件，持久保存结果 | 保存失败不调用模型；核对结束不自动提交或批准 |
...
- 待审论文保留同版本上传快照时，分类解析仍优先采用旧快照。管理员保存新的超导体类别或材料维度后，重新打开编辑页或批准时可能读回旧值；批准载荷与落库值不一致会返回 `409 evidence_stale`。这是当前分类读取缺口，不应通过放宽证据校验绕过；见 [Issue #100 核验记录](../../specs/100-quick-review-status/validation.md)。
