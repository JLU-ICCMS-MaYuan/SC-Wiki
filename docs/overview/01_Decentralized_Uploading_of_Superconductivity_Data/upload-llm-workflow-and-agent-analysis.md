...
初版核对日期：2026-09-18；补充内容核对日期：2026-09-19；层次整理日期：2026-09-19。本文区分当前代码事实、用户已明确的设计要求和待验证的建议。架构、改进与迁移建议不表示已经实现，也不维护后续任务状态。当前功能入口仍是[上传总览](README.md)。

初版只更新文档。全文按“核心结论与架构 → 当前工作方式 → 关键瓶颈与解决方案 → 数据专项设计 → 工具与联网 → 代码和资料依据”组织，保留原有二十节的全部主题与正文内容。

## 2026-09-24 已实现补充：可选 PDF 新链路

默认仍为 legacy，下文原有流程分析对应该默认链路。#114 的可选新方案现已接入
Docling/MinerU/PyMuPDF → Document IR → 版本化模块化科学候选 → Claim 定位与覆盖
检查 → 后端组装草稿。科学值不再经过全文 LLM 二次改写；未知方法保持 unknown，
材料、压力、方法、判据与每条条件分别保存。原生 PDF LLM 适配尚待真实供应商验收。

有限工具循环只修复已有同值候选的出处，不能写科学数据库或无限联网；区域查看
从当前 PDF 按需渲染。提交、Go 审核和返修升版已通过隔离端到端验证，固定模型响应
不代表真实提取质量。Shadow 使用独立 RQ 作业和文件快照，旧链路仍服务用户；批量
评测驱动已能上传并导出实际 AI 产物，但真实 50 篇人工标注质量报告尚缺。

当前实现、验证范围及剩余边界以 [PDF 解析管线](pdf-parsing-pipeline.md) 与
[#114 验证记录](../../specs/114-multimodal-pdf-upload-agent/validation.md) 为准；下文
尚未落地的架构、联网和进一步改进建议仍属于设计讨论。

**阅读导航**

建议先读第一部分建立 Agent 框架，再读第二部分的流程与内容分流、第三部分的提速和证据方案，最后按需查阅字段、来源与代码细节。最关心解析慢和信息遗漏时，可直接进入第三部分。

| 阅读顺序与类型 | 主要问题 | 章节导航 |
| --- | --- | --- |
| [一、核心结论与 Agent 总体架构](#一核心结论与-agent-总体架构) | 先理解目标、职责、设计步骤及与问答的关系 | [1.1 能否抽象](#11-先回答可以抽象成上传-agent-吗)；[1.2 从零设计与科研例子](#12-从零理解并设计一个科研-agent)；[1.3 封装与有限自主行动](#13-如何理解抽象成上传-agent架构分析)；[1.4 上传与问答](#14-上传和网站问答哪些相同哪些不同) |
| [二、当前上传流程与提示词规则](#二当前上传流程与提示词规则) | 看懂现有执行链路、示例和内容分流 | [2.1 论文到表单](#21-从论文到表单实际经过哪些环节)；[2.2 示意数据](#22-用一组示意数据走一遍)；[2.3 提示词职责与调用](#23-那些提示词分别在做什么)；[2.4 内容分流与 Harness](#24-遇到不同论文内容提示词应怎样带着模型往下走) |
| [三、关键瓶颈：效率、完整性与证据](#三关键瓶颈效率完整性与证据) | 重点解决信息压缩、容量限制、漏提取与耗时 | [3.1 现有六项边界](#31-当前实现的能力边界)；[3.2 逐项方案与评估](#32-六项能力边界分别怎样解决)；[3.3 整篇阅读与证据](#33-整篇-pdf-阅读如何兼顾效率和证据) |
| [四、数据契约、Tc 方法与多语言](#四数据契约tc-方法与多语言) | 对齐提取、表单、存储、筛选和显示 | [4.1 统一提取契约](#41-提取结构与表单结构应当统一到哪一层)；[4.2 逐条 Tc 方法](#42-tc-方法自动填写可以保留但要按每条结果确定)；[4.3 原文保存与语言显示](#43-语言原文保存显示跟随系统) |
| [五、工具调用与联网边界](#五工具调用与联网边界) | 弄清工具如何执行、联网何时允许及如何限制 | [5.1 工具执行循环](#51-没有工具列表就不会自动搜索到底是什么意思)；[5.2 联网参与方式](#52-联网搜索应怎样参与建议尚未接入上传)；[5.3 出版信息与术语边界](#53-联网的严格边界只补出版信息与解释术语) |
| [六、代码整理、验证依据与参考资料](#六代码整理验证依据与参考资料) | 需要核实实现或继续研究时查阅 | [6.1 旧提取器精简](#61-旧-extractorpy-应如何精简)；[6.2 代码与验证入口](#62-代码与现有验证依据)；[6.3 调研资料与限制](#63-调研资料可借鉴部分与使用限制) |

**原问题索引**

| 本次问题 | 对应章节 |
| --- | --- |
| 遇到不同论文内容，提示词如何处理 | [第 2.4 节](#24-遇到不同论文内容提示词应怎样带着模型往下走) |
| 整篇 PDF 阅读与逐条证据如何兼顾 | [第 3.3 节](#33-整篇-pdf-阅读如何兼顾效率和证据) |
| 提取结构与当前表单如何统一 | [第 4.1 节](#41-提取结构与表单结构应当统一到哪一层) |
| Tc 方法自动填写及图表筛选 | [第 4.2 节](#42-tc-方法自动填写可以保留但要按每条结果确定) |
| 原语言保存、按网站语言显示 | [第 4.3 节](#43-语言原文保存显示跟随系统) |
| 工具列表与工具调用循环是什么 | [第 5.1 节](#51-没有工具列表就不会自动搜索到底是什么意思) |
| 是否移除旧 extractor.py | [第 6.1 节](#61-旧-extractorpy-应如何精简) |
| 六项能力边界各自怎样解决 | [第 3.2 节](#32-六项能力边界分别怎样解决) |
| 从零设计科研 Agent 与本项目架构 | [第 1.2 节](#12-从零理解并设计一个科研-agent) |
| 联网只补出版信息与解释术语 | [第 5.2 节](#52-联网搜索应怎样参与建议尚未接入上传)、[第 5.3 节](#53-联网的严格边界只补出版信息与解释术语) |
| 开源项目与论文调研依据 | [第 6.3 节](#63-调研资料可借鉴部分与使用限制) |

## 一、核心结论与 Agent 总体架构

先建立整体认识：上传 Agent 要交付什么、怎样设计、与现有问答怎样共用能力。其中 1.1 和 1.4 解释现有实现，1.2 和 1.3 提出架构建议；建议不表示已经落地。

### 1.1 先回答：可以抽象成上传 Agent 吗？
...
### 1.2 从零理解并设计一个科研 Agent

#### 1.2.1 先把它理解成“有工作目标、工具和反馈的执行系统”
...
### 1.3 如何理解“抽象成上传 Agent”？（架构分析）

#### 1.3.1 第一层：把现有能力封装为一个任务入口
...
沿一篇论文从输入到草稿的路径，理解每个提示词在何时生效、遇到不同内容怎样处理。2.1～2.3 说明当前行为；2.4 先整理现有内容规则，再讨论 Harness 应增加的行动规则。
...
归一化还包括枚举处理、原始数值与范围解析、元素数计算、空间群号与晶系映射等。2026-09-19 复核发现，初版描述的 `_apply_methodology_inference` 已从当前代码移除；当前汇总提示要求论文级机制分类有明确依据，并要求缺少逐条对应证据的 Tc 方法保持 `unknown`。恢复有依据的逐条 Tc 方法自动填写是本次提出的设计要求，详见[第 4.2 节](#42-tc-方法自动填写可以保留但要按每条结果确定)。
...
### 2.4 遇到不同论文内容，提示词应怎样带着模型往下走？

#### 2.4.1 当前流程的“分流”发生在哪里？
...
#### 2.4.2 Harness 应该补上的，是可执行的行动规则

这里把 Harness 理解为“让模型能可靠工作的运行系统”：它给模型准备材料、工具、状态和预算，执行行动、检查结果，并决定继续、重试或交给人。这个词的用法并不完全统一，不是一种必须购买或训练的新模型。

提示词是其中一部分。重要边界应由程序执行，例如“联网工具不能把数值写入 Tc”，不能只靠模型记住一句禁令。Anthropic 的工程文章和 LangGraph 文档都区分了固定工作流与动态工具选择；二者可以组合使用，见[第 6.3 节](#63-调研资料可借鉴部分与使用限制)资料 [1]、[2]。
...
优先关注会影响提取可信度和使用效率的问题：先列现有六项边界，再逐项给出解决思路，最后展开整篇或大节阅读与逐条证据兼顾的方案。这里的改进方案尚未实施或测出效果。
...
以下方案来自对现有实现的分析，以及[第 6.3 节](#63-调研资料可借鉴部分与使用限制)列出的开源资料和论文；没有在本项目中实施或测出效果。
...
#### 3.2.1 重点解决第 2 项：不要让“摘要”成为事实的唯一载体
...
### 3.3 整篇 PDF 阅读，如何兼顾效率和证据？

#### 3.3.1 最关键的调整：把阅读粒度与证据粒度分开
...
集中说明数据怎样从提取结果进入表单，以及方法和语言应遵循什么规则。每节分别保留当前代码事实、用户已明确的要求和迁移建议，避免把目标行为当作现状。

### 4.1 提取结构与表单结构，应当统一到哪一层？
...
先解释工具调用为何需要程序执行循环，再说明联网参与方式和严格边界。联网仅用于出版信息核实及术语解释；这些联网工具尚未接入当前上传流程。

### 5.1 “没有工具列表，就不会自动搜索”到底是什么意思？
...
2026-09-19 的来源核对提示词已区分“原文支持判断”和“单独标记的通用知识候选”：前者仍只能使用当前论文及附件，后者不能标为原文支持。这并不代表已接入联网搜索。按本次明确的边界，联网仅用于出版信息核实与术语解释；Tc 和未上传附件不进入联网查找范围，见[第 5.3 节](#53-联网的严格边界只补出版信息与解释术语)。

**建议的判断标准是：Agent 是否更完整地找到有出处的数据、更准确地区分条件和来源，并降低人工修正量。** 是否使用 Agent 框架、提示词是否更长，都不能单独证明提取质量变好。

### 5.3 联网的严格边界：只补出版信息与解释术语

#### 5.3.1 老论文没有 DOI，不代表它没有可核实身份
...
最后查阅旧提取器精简建议、现有代码入口和调研来源。代码事实与外部资料的验证范围、尚未测量的效果及实施前提均保留在本部分。

### 6.1 旧 extractor.py 应如何精简？
