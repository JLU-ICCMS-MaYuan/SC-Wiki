...
疑点中的原文默认折叠，可按需查看；不显示内部文件或片段编号，只保留页码信息，缺页码时明确说明。
「让系统重新查找」会经过可取消倒计时，实际重新核对未解决记录，不直接返回旧疑点缓存。

核对任务完成后，上传目标会自动将已定位证据临时写回 Redis 草稿。用户返回编辑页或离开后再进入时，
该证据和已完成任务可复用，不会重复调用模型；缺失出处不写入临时草稿。正式提交时服务端仍按版本和权限
重新校验，并在论文事务内把证据与核对结果写入 MySQL。
