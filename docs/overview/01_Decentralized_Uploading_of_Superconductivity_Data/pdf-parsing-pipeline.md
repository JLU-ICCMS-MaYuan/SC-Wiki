...
创建任务 API 已接受 `parser_profile`。默认配置为 `legacy`；显式选择 `layout` 使用
Docling 2.130.0 的 CPU 文本层/表格路径，`ocr` 使用 MinerU 4.0.6 的 basic OCR，
`text` 使用 PyMuPDF。TXT/MD 与 ASE 结构附件保留原路由。此处没有前端方案选择控件，
`vision` 和 PDF 原生 LLM 供应商尚未接通；这些选项不能视为可运行能力。

Docling/MinerU 在独立子进程运行，默认 300 秒超时后终止进程组；失败保留解析错误码，
不切换到其他方案。成功结果先保存任务级 Document IR，再转换为带页码的文本供既有
分段读取。新方案不复用旧 Markdown；分段缓存同时检查方案和原文摘要，避免换方案后
沿用旧结果。区域坐标统一为左上原点的 PDF 点坐标；MinerU HTML 表格无单元格区域时
仅保留行列、跨度和限制，不编造 bbox。

实际验证包括数字 PDF、空白页、合成扫描 PDF、Docling 原生类型映射与 MinerU 表格
映射；尚不代表复杂超导论文质量门通过。正式文档定位事务、前端区域证据、完整 Claim
和覆盖审计链路及 Shadow/灰度验收仍未完成。当前功能与验证边界见
[#114 验证记录](../../specs/114-multimodal-pdf-upload-agent/validation.md)。
