...
研究者可以独立提问、回答，在答案、体系和论文下评论或回复。所有业务接口由 Go 提供；Python 仅登记数据库元数据。

## 当前行为

- 侧栏社区展开 `/share/rankings`、`/share/charts`、`/share/discussions`。旧 `/share` 重定向至图表。问题列表支持关键词、最新和最近活跃排序；答案默认按赞同数排序，也可按最新排序，不设采纳。
- 问题与答案支持 Markdown、链接、代码、公式和预览，不执行原始 HTML，也不渲染图片。评论、回复为纯文本。
- 元素搜索提供 `/systems/:systemKey` 入口，例如 Hg。元素符号区分大小写，组合去重后按字母顺序生成稳定标识；不同论文共用同一体系讨论，页面仅列出该体系当前公开论文。
- 独立论文详情、搜索论文详情和图表论文抽屉共用 `PaperCommunity`。体系页及三个论文入口各保留一个主评论输入区，回复时展开对应回复输入区，不再展示弹幕控件或发起弹幕轮询。论文评论按论文 ID 关联，修改版本不丢失评论；体系评论与论文评论互不混合。
- 评论按根评论归组，回复在根评论下平铺并标明回复对象。分页补上根评论上下文，通知跳转能定位到长线程中目标回复所在页。
- 正常且邮箱验证通过的账号可直接发布，游客可阅读公开内容。作者可编辑问题、答案、评论并删除自己的内容。评论隐藏或删除后留下无正文占位及已有回复，不能继续回复该评论；问题或答案不可见时，后代也不可访问。
...
`community_systems` 保存独立体系空间；`community_entries` 保存有明确目标外键的问题、答案、评论三类当前内容；点赞、举报、处理事件、通知分别保存。论文物理删除时级联清理对应互动，体系讨论保留。

论文互动复用论文可见性规则，通知读取时重新检查内容和祖先权限，不保存正文快照。写入在事务中按祖先到后代锁定并读取当前权限，避免隐藏与回复并发时继续向失效目标发布。点赞使用唯一约束保持幂等。

请求体上限 128 KiB；标题 3–200 字，问题及答案正文最多 20000 字，评论 2000 字。账号与 IP 通过 Redis 限流，Redis 不可用时拒绝写入。对外只展示用户名、头像和账号状态标记，不返回邮箱；注销账号匿名显示。

## 约束

- 当前实现不包含图片附件、私信、关注、推荐或实时长连接。
- 弹幕功能已取消，体系及论文详情仅保留评论区。通过身份与频率校验后，旧客户端的弹幕创建和列表请求返回 400，历史弹幕按 ID 读取、操作及焦点定位返回 404。历史弹幕留库，不转为评论；通知列表、未读数和待处理/已处理举报列表均排除这些记录。取消弹幕不改变数据库结构。
- 迁移 `20260917_0107` 依赖 `20260916_0106`，2026-09-17 已经用户确认应用至本地 `scwiki`；六张社区表已建立，当前 Go/Vite 读取与页面恢复正常，实际数据库写入回滚检查通过。用户已确认当前账号在原页面发布成功；这不代表其他环境已部署，见[本地启用验收](../../specs/106-community-discussion/quickstart.md)。
